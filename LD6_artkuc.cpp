...
public:
    ...
    void FeelBad() const;
    friend ostream& operator << (ostream& O, const Human& human);
};
...
void Firm::operator+=(const Employee& employee) {
...
ostream& operator << (ostream& O, const Firm& firm) {
    for (unsigned int i = 0; i < firm.curLen; i++) {
        O << (i + 1) << ". " << *(firm.employees[i]) << endl;
    }
    return O;
}
...
ostream& operator << (ostream& O, const Employee& employee) {
    O << (Human&)employee << ". Salary = " << employee.alga;
    return O;
}
...
ostream& operator << (ostream& O, const Human& human) {
    O << "gender = " << human.gender << ". name = " << human.name << ". birth_year = " << human.birth_year;
    return O;
}
